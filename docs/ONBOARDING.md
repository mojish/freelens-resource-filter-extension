...
│   ├── components/         # ResourceFilterPage, FilterBar, styles
│   ├── discovery/          # resource-kind discovery via apiManager
│   └── filtering/          # filter engine, field-path collector, MobX store
...
4. Pack a tarball: `pnpm pack`
5. Install into Freelens: drag the `*.tgz` onto the Freelens Extensions page. If the enable/disable loop appears, restart Freelens. It is a known host bug.
6. First code to read: `src/renderer/index.tsx` (registration), `src/renderer/components/resource-filter-page.tsx` (the page), `src/renderer/filtering/filter-engine.ts` (operator semantics).
